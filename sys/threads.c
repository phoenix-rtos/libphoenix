...
typedef struct _mutex_t {
	...
	_Atomic(int) owner;
	_Atomic(uint32_t) recursive_counter;
	int type;
	idnode_t linkage;
} mutex_t;
...
int mutexCreateWithAttr(handle_t *h, const struct lockAttr *attr)
{
	...
	*((int *)h) = (INT_MAX / 2) + idtree_alloc(&uresource_common.mutexes, &mutex->linkage);
	spinlock_unlock(&uresource_common.lock);
	return EOK;
}
...
int condCreateWithAttr(handle_t *h, const struct condAttr *attr)
{
	...
	*((int *)h) = (INT_MAX / 2) + idtree_alloc(&uresource_common.conds, &cond->linkage);
	spinlock_unlock(&uresource_common.lock);
	return EOK;
}
...
int condWait(handle_t h, handle_t m, time_t timeout)
{
	...
	int ret;
	do {
		ret = phFutexWait(&cond->value, value, timeout, cond->clock);
	} while (ret == -EINTR);
	_mutexLock(mutex);
	if (ret == -ETIME) {
		return ret;
...
int condBroadcast(handle_t h)
{
	...
	_condWakeupN(cond, FUTEX_WAKEUP_ALL);
	return EOK;
}
...
int interrupt(unsigned int n, int (*f)(unsigned int, void *), void *arg, handle_t cond_handle, handle_t *handle)
{
	...
	return phInterrupt(n, f, arg, cond == NULL ? NULL : &cond->value, handle);
}
