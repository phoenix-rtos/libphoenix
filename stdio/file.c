...
static ssize_t full_read(int fd, void *ptr, size_t size, int *eof)
{
	ssize_t err;
	ssize_t total = 0;
...
{
	int eof;
	ssize_t err = full_read(stream->fd, stream->buffer, stream->bufsz, &eof);
	if (err >= 0) {
		if (err > 0) {
			stream->bufpos = 0;
			stream->bufeof = err;
		...
		if (err < readsz) {
			/* EOF or EAGAIN */
			stream->flags |= (eof == 1) ? F_EOF : F_ERROR;
		}
	}
	else {
		stream->flags |= F_ERROR;
...
{
	int eof;
	ssize_t err = full_read(stream->fd, ptr, readsz, &eof);
	if (err >= 0) {
		if (err < readsz) {
			/* EOF or EAGAIN */
			stream->flags |= (eof == 1) ? F_EOF : F_ERROR;
