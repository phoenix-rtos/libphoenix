...
static inline int valid_hex(const char *str)
{
	return isxdigit(str[0]) || ((str[0] == DECIMAL_POINT) && isxdigit(str[1]));
}


static inline int valid_decimal(const char *str)
{
...
static const char *floatparse_zerox(const char *str, bool *is_hex)
{
	*is_hex = 0;
	if (*str == '0') {
		str++;
		if (((*str == 'x') || (*str == 'X')) && valid_hex(str + 1)) {
			str++;
			*is_hex = 1;
		}
...
static const char *scan_number(const char *str, bool hex, bool allow_dec, uint64_t limit, size_t *n_decimal, size_t *n_overflowed, uint64_t *out)
{
...
static const char *scan_number_long_double(const char *str, bool hex, size_t digit_limit, size_t *n_decimal, size_t *n_overflowed, long double *out)
{
	const int base = hex ? 16 : 10;
...
#if __LDBL_MAX_10_EXP__ < (1 << POW10_LOOKUPS)
...
static long double mul_exp2(long double in, int64_t exp)
{
	...
	conv_t multiplier = { .d = 0.0 };
	multiplier.i.exponent = 0x3ff + exp;
	return in * multiplier.d;
}


/* Parse through the number and exponent */
static const char *floatparse_number(const char *str, bool hex, uint8_t type, long double *out)
{
	int32_t exp_max;
	int32_t exp_min;
	...
	switch (type) {
		case TYPE_ID_FLT:
			if (hex) {
				exp_min = __FLT_MIN_EXP__ - 64;
				exp_max = __FLT_MAX_EXP__ + 64;
			}
			else {
				exp_min = __FLT_MIN_10_EXP__ - UINT64_MAX_DEC_DIGITS;
				exp_max = __FLT_MAX_10_EXP__ + UINT64_MAX_DEC_DIGITS;
			}
			break;

		case TYPE_ID_DBL:
			if (hex) {
				exp_min = __DBL_MIN_EXP__ - 64;
				exp_max = __DBL_MAX_EXP__ + 64;
			}
			else {
				exp_min = __DBL_MIN_10_EXP__ - UINT64_MAX_DEC_DIGITS;
				exp_max = __DBL_MAX_10_EXP__ + UINT64_MAX_DEC_DIGITS;
			}
			break;

		default: /* TYPE_ID_LDBL */
			if (hex) {
				exp_min = __LDBL_MIN_EXP__ - __LDBL_MANT_DIG__;
				exp_max = __LDBL_MAX_EXP__ + __LDBL_MANT_DIG__;
			}
			else {
				exp_min = __LDBL_MIN_10_EXP__ - LDBL_MANT_10_DIG;
				exp_max = __LDBL_MAX_10_EXP__ + LDBL_MANT_10_DIG;
			}
			break;
	}
	...
	if (type <= TYPE_ID_DBL) {
		...
		uint64_t digits;
		str = scan_number(str, hex, true, limit, &frac_position, &overflow_int, &digits);
		result = (long double)digits;
	}
	else {
		str = scan_number_long_double(str, hex, LDBL_MANT_10_DIG, &frac_position, &overflow_int, &result);
	}
	...
	if (has_exponent) {
		...
		if (isdigit(*str_exp)) {
			...
			uint64_t exp_scan;
			str = scan_number(str_exp, 0, false, ((INT32_MAX - 9) / 10), &dummy, &exp_overflowed, &exp_scan);
			if (exp_overflowed != 0) {
...
static long double strto_any(const char *__restrict str, char **__restrict endptr, uint8_t type)
{
	...
	bool is_hex;
	const char *str_zerox = floatparse_zerox(str_wsin, &is_hex);
	if (str_zerox == NULL) {
		errno = EINVAL;
		if (endptr != NULL) {
			*endptr = (char *)str;
	...
	str = floatparse_number(str_zerox, is_hex, type, &result);

	bool range_error;
	switch (type) {
		case TYPE_ID_FLT: {
			float cast = (float)result;
			range_error = isinf(cast) || ((result != 0) && (cast == 0));
			break;
		}
