...
static inline bool valid_hex(const char *str, int decimal_point)
{
	return isxdigit(str[0]) || ((str[0] == decimal_point) && isxdigit(str[1]));
}


static inline bool valid_decimal(const char *str, int decimal_point)
{
...
static const char *floatparse_zerox(const char *str, int decimal_point, bool *is_hex)
{
	*is_hex = 0;
	if (*str == '0') {
		str++;
...
static const char *scan_number(const char *str, bool hex, int decimal_point, uint64_t limit, size_t *n_decimal, size_t *n_overflowed, uint64_t *out)
{
...
static const char *scan_number_long_double(const char *str, bool hex, int decimal_point, size_t digit_limit, size_t *n_decimal, size_t *n_overflowed, long double *out)
{
	const int base = hex ? 16 : 10;
...
static const char *floatparse_number(const char *str, bool hex, uint8_t type, int decimal_point, long double *out)
{
	int32_t exp_max;
	int32_t exp_min;
...
static long double strto_any(const char *__restrict str, char **__restrict endptr, uint8_t type)
{
	...
	int decimal_point = decimal_point_str[0];
	if ((decimal_point == 0) || (decimal_point_str[1] != 0)) {
