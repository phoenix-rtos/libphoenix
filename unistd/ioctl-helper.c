...
#define IOCTL_PACK(type, out_ptr, ptr, sub_ptr_name, sub_struct_size, total_size) \
	do { \
		...
		*out_ptr = _tmp; \
		memcpy(_tmp, ptr, sizeof(type)); \
		_tmp = (char *)_tmp + sizeof(type); \
		memcpy(_tmp, ptr->sub_ptr_name, sub_struct_size); \
	} while (0)

#define IOCTL_UNPACK(type, request, ioctl_struct, in_ptr, size, sub_ptr_name) \
	do { \
		type *_tmp = ioctl_struct; \
		memcpy(_tmp->sub_ptr_name, (char *)in_ptr + sizeof(type), size - sizeof(type)); \
		memcpy(_tmp, in_ptr, sizeof(type)); \
		_tmp->sub_ptr_name = (char *)in_ptr + sizeof(type); \
	} while (0)
...
static inline ssize_t sockioctl_packEthtool(const struct ifreq *ifr, void **out_ptr, size_t *out_size)
{
	...
	switch (cmd) {
		...
		case ETHTOOL_PERQUEUE:
			/* sub_command only ever used as ETHTOOL_[G/S]COALESCE */
		...
		case ETHTOOL_GRXNTUPLE: /* not handled by ethtool? */
		default:
			return -ENOSYS;
	}

	SOCKIOCTL_PACK(struct ifreq, out_ptr, ifr, ifr_data, ethtool_struct_size, out_size);
...
static inline int ioctl_packRtEntry(const struct rtentry *rt, void **out_ptr, size_t *out_size)
{
	...
	IOCTL_PACK(struct rtentry, out_ptr, rt, rt_dev, rt_dev_size, out_size);
...
static inline int ioctl_packIfconf(const struct ifconf *ifc, void **out_ptr, size_t *out_size)
{
	...
	IOCTL_PACK(struct ifconf, out_ptr, ifc, ifc_buf, ifc_buf_len, out_size);
...
int ioctl_serialize(unsigned long request, void *ioctl_val, void **out_ptr, size_t *out_size)
{
	...
	*out_ptr = NULL;
	...
	switch (request) {
		...
		case SIOCETHTOOL:
			ret = sockioctl_packEthtool(ioctl_val, out_ptr, out_size);
			break;
...
void ioctl_deserialize(unsigned long request, void *ioctl_val, void *in_ptr, size_t size)
{
	switch (request) {
		case SIOCADDRT:
		case SIOCDELRT:
			IOCTL_UNPACK(struct rtentry, request, ioctl_val, in_ptr, size, rt_dev);
			break;

		case SIOCGIFCONF:
			IOCTL_UNPACK(struct ifconf, request, ioctl_val, in_ptr, size, ifc_buf);
			break;

		case SIOCETHTOOL:
			SOCKIOCTL_UNPACK(struct ifreq, request, ioctl_val, in_ptr, size, ifr_data);
			break;
