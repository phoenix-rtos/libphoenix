...
{
	if (grp_common.lock != -1) {
		resourceDestroy(grp_common.lock);
	}
...
{
	/* cap the realloc */
	if (grp_common.size == GRP_MAX_BUSFIZE) {
		return -ENOMEM;
	}

	size_t newSize = min(GRP_MAX_BUSFIZE, max(GRP_DEFAULT_BUFSIZE, grp_common.size + grp_common.size / 2));
...
static int getlinematch(FILE *fp, const char *name, gid_t *gid, struct group *grp, char *buffer, size_t bufsize, struct group **result, bool staticBuf)
{
	...
	if (staticBuf) {
		if (grp_common.lock == -1 && mutexCreate(&grp_common.lock) < 0) {
			return ENOMEM;
		}
		mutexLock(grp_common.lock);
	}
